"""
Runner for ABLATION 4.3.3 (Laplacian heterogeneous). Mirrors run_hetero_fast.py's
sweep-then-multiseed structure exactly, but calls
ablation_laplacian_hetero.run_befgc_hetero_laplacian instead of the GW-based
befgc_hetero.run_befgc_hetero. Does not import or modify run_hetero_fast.py.

Usage (same CLI as run_hetero_fast.py):
    python run_ablation_laplacian.py --datasets IMDB --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
from ablation_laplacian_hetero import run_befgc_hetero_laplacian
...
def sweep(ds, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        for sd in range(obj_seeds):
            result = run_befgc_hetero_laplacian(
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [laplacian sweep {t+1:02d}/{max_evals}] mean={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="ablation_laplacian.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[laplacian] device={device}, datasets={args.datasets}, ratio={args.ratio}, "
    ...
    for dataset in args.datasets:
        ...
        print(f"\n[laplacian] === {dataset} === node_counts={node_counts} "
        ...
        print(f"[laplacian] {dataset} best robust score={best_acc*100:.2f}% "
