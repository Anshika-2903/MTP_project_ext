"""
Full link-prediction run for LastFM (a link-prediction-only dataset, no
node-classification labels -- see data_loading_new.py). Identical to
run_linkpred.py except it loads via `load_lastfm()` instead of
`load_hetero(name)`, since LastFM isn't in data_loading_hetero.py's LOADERS
registry (kept separate deliberately -- see data_loading_new.py's
docstring). Does not modify run_linkpred.py or any other existing file.

Usage:
    python run_linkpred_lastfm.py --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred-lfm sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred_gw_LastFM.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred-lfm] device={device}, ratio={args.ratio}, "
    ...
    print(f"\n[linkpred-lfm] === LastFM === rel={split['rel']} "
    ...
    print(f"[linkpred-lfm] LastFM best robust AUC={best_auc*100:.2f}% "
    ...
    for sd in range(args.n_seeds):
        ...
        print(f"[linkpred-lfm]   seed {sd}: " + " | ".join(row) +
    ...
    print(f"[linkpred-lfm] === LastFM RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
    for m in MODELS:
        ...
        print(f"[linkpred-lfm]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
              f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
    with open(args.out_jsonl, "a") as f:
        f.write(json.dumps(rec) + "\n")

    print("\n[linkpred-lfm] ALL DONE.", flush=True)
