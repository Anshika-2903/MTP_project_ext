"""
Runner for link prediction (1.1) using the INDEPENDENT-GW coarsening
(ablation 4.3.1: each node type coarsened as its own fully independent
BEFGC problem, no joint cross-type optimization) instead of full GW-based
BEFGC. Byte-for-byte identical to run_linkpred.py except for which
coarsening function builds Ac/Xc_dict -- isolates whether joint cross-type
coupling matters for link prediction the way it did for node classification
(where independent-GW roughly halved SGC's accuracy on all 3 datasets).

Usage:
    python run_linkpred_independent_gw.py --datasets IMDB --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
from ablation_independent_gw import run_befgc_independent_gw as run_befgc_hetero
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        for sd in range(obj_seeds):
            ...
            # Independent-GW's history is a dict {type: [J values]} (one
            # trajectory per type), not a flat list like the joint methods.
            diverged_any |= any(h[-1] > h[0] + 1e-6 for h in result["history"].values())
            aucs = []
            for m in MODELS:
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred-indep sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred_independent_gw.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred-indep] device={device}, datasets={args.datasets}, ratio={args.ratio}, "
    ...
    for dataset in args.datasets:
        ...
        print(f"\n[linkpred-indep] === {dataset} === rel={split['rel']} "
        ...
        print(f"[linkpred-indep] {dataset} best robust AUC={best_auc*100:.2f}% "
        ...
        for sd in range(args.n_seeds):
            ...
            print(f"[linkpred-indep]   seed {sd}: " + " | ".join(row) +
                  f" empty={result['empty_supernodes']}", flush=True)

        rec = {"dataset": dataset, "ratio": args.ratio, "task": "link_prediction",
               "coarsening": "laplacian", "rel": list(split["rel"]), "best_params": best_params,
               "avg_empty": float(np.mean(empties)), "n_seeds": args.n_seeds,
               "mins": (time.time() - t0) / 60, "ts": time.strftime("%Y-%m-%d %H:%M:%S")}
        print(f"[linkpred-indep] === {dataset} RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
        for m in MODELS:
            ...
            print(f"[linkpred-indep]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
                  f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
        with open(args.out_jsonl, "a") as f:
            f.write(json.dumps(rec) + "\n")

    print("\n[linkpred-indep] ALL DONE.", flush=True)
