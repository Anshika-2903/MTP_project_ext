"""
Link prediction on LastFM using the NO-CROSS-TYPE coarsening (ablation
4.3.2), targeting the user-user relation specifically (via an explicit
`rel=` override to sample_link_split_and_mask) rather than the default
highest-edge-count relation (user-artist).

This is deliberate: no-cross-type zeroes all CROSS-type edges before
coarsening, so it can never predict a cross-type relation (user-artist,
artist-tag) -- there's nothing left to learn (see link_pred_hetero.py and
ICLR_RESULTS.md section 2.3 for the "0 coarse pairs" failure this produces
on cross-type relations). user-user is INTRA-type, so it survives the
ablation untouched, making this the one legitimate no-cross-type
link-prediction experiment. Does not modify any other existing file.

Usage:
    python run_linkpred_lastfm_nocross_useruser.py --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
from data_loading_new import load_lastfm
from ablation_no_cross_type import run_befgc_no_cross_type as run_befgc_hetero
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred-lfm-nocross-uu sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred_nocross_useruser_LastFM.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred-lfm-nocross-uu] device={device}, ratio={args.ratio}, "
    ...
    split = sample_link_split_and_mask(ds, rel=('user','to','user'), val_frac=args.val_frac,
                                       test_frac=args.test_frac, seed=0)
    print(f"\n[linkpred-lfm-nocross-uu] === LastFM === rel={split['rel']} "
    ...
    print(f"[linkpred-lfm-nocross-uu] LastFM best robust AUC={best_auc*100:.2f}% "
    ...
    for sd in range(args.n_seeds):
        ...
        print(f"[linkpred-lfm-nocross-uu]   seed {sd}: " + " | ".join(row) +
              f" empty={result['empty_supernodes']}", flush=True)

    rec = {"dataset": "LastFM", "ratio": args.ratio, "task": "link_prediction",
    ...
    print(f"[linkpred-lfm-nocross-uu] === LastFM RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
    for m in MODELS:
        ...
        print(f"[linkpred-lfm-nocross-uu]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
              f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
    with open(args.out_jsonl, "a") as f:
        f.write(json.dumps(rec) + "\n")

    print("\n[linkpred-lfm-nocross-uu] ALL DONE.", flush=True)
