"""
Full link-prediction run for LastFM using the LAPLACIAN-SMOOTHNESS
coarsening (ablation 4.3.3) instead of full GW-based BEFGC, on LastFM's
default (highest-edge-count) relation: user-artist. Isolates whether GW's
extra structural sophistication matters on LastFM specifically (it doesn't --
near-identical to GW, see ICLR_RESULTS.md). Identical to
run_linkpred_lastfm.py except for which coarsening function builds
Ac/Xc_dict. Does not modify run_linkpred_lastfm.py or any other existing file.

Usage:
    python run_linkpred_lastfm_laplacian.py --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred-lfm-lap sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred_lap_LastFM.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred-lfm-lap] device={device}, ratio={args.ratio}, "
    ...
    print(f"\n[linkpred-lfm-lap] === LastFM === rel={split['rel']} "
    ...
    print(f"[linkpred-lfm-lap] LastFM best robust AUC={best_auc*100:.2f}% "
    ...
    for sd in range(args.n_seeds):
        ...
        print(f"[linkpred-lfm-lap]   seed {sd}: " + " | ".join(row) +
    ...
    print(f"[linkpred-lfm-lap] === LastFM RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
    for m in MODELS:
        ...
        print(f"[linkpred-lfm-lap]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
              f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
    with open(args.out_jsonl, "a") as f:
        f.write(json.dumps(rec) + "\n")

    print("\n[linkpred-lfm-lap] ALL DONE.", flush=True)
