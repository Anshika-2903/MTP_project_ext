"""
Runner for downstream task 1.1 (Link prediction: ROC-AUC, AP).

Same sweep-then-multiseed structure as run_hetero_fast.py, with two changes
required for link prediction specifically:
  1. The edge split + leakage-free adjacency mask (`sample_link_split_and_mask`)
     is built ONCE per dataset, BEFORE coarsening -- the coarsening itself must
     run on the masked adjacency so val/test edges never leak into Ac/Xc_dict.
  2. The sweep/eval objective is mean ROC-AUC over `obj_seeds` coarsening
     seeds and all three backbones (mirrors run_hetero_fast.py's accuracy
     objective, just swapping in AUC).

Usage:
    python run_linkpred.py --datasets IMDB --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred] device={device}, datasets={args.datasets}, ratio={args.ratio}, "
    ...
    for dataset in args.datasets:
        ...
        print(f"\n[linkpred] === {dataset} === rel={split['rel']} "
        ...
        print(f"[linkpred] {dataset} best robust AUC={best_auc*100:.2f}% "
        ...
        for sd in range(args.n_seeds):
            ...
            print(f"[linkpred]   seed {sd}: " + " | ".join(row) +
        ...
        print(f"[linkpred] === {dataset} RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
        for m in MODELS:
            ...
            print(f"[linkpred]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
                  f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
        with open(args.out_jsonl, "a") as f:
            f.write(json.dumps(rec) + "\n")

    print("\n[linkpred] ALL DONE.", flush=True)
