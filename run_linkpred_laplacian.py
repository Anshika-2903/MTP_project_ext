"""
Runner for link prediction (1.1) using the LAPLACIAN-SMOOTHNESS coarsening
(ablation 4.3.3) instead of full GW-based BEFGC. Byte-for-byte identical to
run_linkpred.py except for which coarsening function builds Ac/Xc_dict --
this isolates whether GW's extra structural sophistication over plain
Laplacian smoothness actually matters for link prediction specifically
(node classification at r=0.3 showed almost no difference; testing here
whether an edge-level task is more sensitive to it).

Usage:
    python run_linkpred_laplacian.py --datasets IMDB --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
from ablation_laplacian_hetero import run_befgc_hetero_laplacian as run_befgc_hetero
...
def sweep(ds, split, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [linkpred-lap sweep {t+1:02d}/{max_evals}] mean_auc={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="linkpred_laplacian.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[linkpred-lap] device={device}, datasets={args.datasets}, ratio={args.ratio}, "
    ...
    for dataset in args.datasets:
        ...
        print(f"\n[linkpred-lap] === {dataset} === rel={split['rel']} "
        ...
        print(f"[linkpred-lap] {dataset} best robust AUC={best_auc*100:.2f}% "
        ...
        for sd in range(args.n_seeds):
            ...
            print(f"[linkpred-lap]   seed {sd}: " + " | ".join(row) +
        ...
        print(f"[linkpred-lap] === {dataset} RESULT (avg_empty={np.mean(empties):.1f}) ===", flush=True)
        for m in MODELS:
            ...
            print(f"[linkpred-lap]   {m:11s}: AUC={a.mean():.2f}+/-{a.std():.2f}  "
                  f"AP={p_.mean():.2f}+/-{p_.std():.2f}", flush=True)
        with open(args.out_jsonl, "a") as f:
            f.write(json.dumps(rec) + "\n")

    print("\n[linkpred-lap] ALL DONE.", flush=True)
