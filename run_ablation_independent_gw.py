"""
Runner for ABLATION 4.3.1 (Independent GW). Mirrors run_hetero_fast.py's
sweep-then-multiseed structure exactly, but calls
ablation_independent_gw.run_befgc_independent_gw instead of the joint
befgc_hetero.run_befgc_hetero. Does not import or modify run_hetero_fast.py.

Usage (same CLI as run_hetero_fast.py):
    python run_ablation_independent_gw.py --datasets IMDB --ratio 0.3 --he_init
"""
import argparse
import json
import time
...
from ablation_independent_gw import run_befgc_independent_gw
...
def _diverged(history_per_type):
    """history is a dict {type_name: [J values]} for this ablation (one
    independent trajectory per type), unlike the joint method's flat list."""
    return any(h[-1] > h[0] + 1e-6 for h in history_per_type.values())
...
def sweep(ds, type_order, node_counts, coarse_counts, max_evals, iters,
    ...
    for t in range(max_evals):
        ...
        for sd in range(obj_seeds):
            result = run_befgc_independent_gw(
            ...
            diverged_any |= _diverged(result["history"])
            accs = []
            for m in MODELS:
        ...
        if score > best_score and not diverged_any:
            ...
            tag = "  <-- best"
        print(f"  [indep-gw sweep {t+1:02d}/{max_evals}] mean={score*100:5.2f}% "
...
def main():
    ...
    p.add_argument("--out_jsonl", default="ablation_independent_gw.jsonl")
    args = p.parse_args()

    coarse_dtype = torch.float64 if args.coarse_dtype == "float64" else torch.float32
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[indep-gw] device={device}, datasets={args.datasets}, ratio={args.ratio}, "
    ...
    for dataset in args.datasets:
        ...
        print(f"\n[indep-gw] === {dataset} === node_counts={node_counts} "
        ...
        print(f"[indep-gw] {dataset} best robust score={best_acc*100:.2f}% "
